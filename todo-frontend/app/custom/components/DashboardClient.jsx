...
import DashboardHeader from '../components/WorkspaceHeader';
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // We ONLY fetch Task and Group data here. User data is handled by useUser()
                const promises = [api.get(taskUrl)];
                if (activeGroupId) {
                    promises.push(api.get(`groups/${activeGroupId}/`));
                }

                const [taskRes, groupRes] = await Promise.all(promises);

                setTasks(taskRes.data.results || taskRes.data);
                if (groupRes) setGroupData(groupRes.data);
